...
class UserManagement(object):
    ...
    def create_user(self, **kwargs):
        ...
        try:
            ...
            if not full_name:
                return {"status": "failed", "message": "Missing full name"}
            email = kwargs.get("email")
            if not email:
                return {"status": "failed", "message": "Missing email"}
            phone_number = kwargs.get("phone_number")
            if not phone_number:
                return {"status": "failed", "message": "Missing phone number"}
            ...
            if not user:
                return {"status": "failed", "message": "Failed creating user"}
            # Set user password
            user.set_password(password)
            # Create user wallet
            user_wallet = UserWallet.objects.get(user=user)
            if user_wallet:
                return {"status": "failed", "message": "User wallet already exists"}
            user_wallet = UserWallet.objects.create(
                user=user, wallet_balance=0.0, wallet_currency="KES", account_number=phone_number
            )
            if not user_wallet:
                return {"status": "failed", "message": "Failed creating user wallet"}
            return {"code": "100.000.000", "status": "success", "context": "User created successfully"}
        except Exception as e:
            pass  # lgr.exception('%s Service create exception: %s' % (self.manager.model.__name__, e))
        return {"code": "500.001.012", "status": "failed", "context": "Failed creating user"}
